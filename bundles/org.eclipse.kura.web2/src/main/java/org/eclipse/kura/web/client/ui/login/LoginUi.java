...
public class LoginUi extends Composite {
    ...
    public LoginUi() {
        ...
        for (String localeName : localeNames) {
            if (!localeName.equals("default")) {
                ...
                this.languageBox.addItem(nativeName, localeName);
                if (localeName.equals(currentLocale)) {
                    this.languageBox.setSelectedIndex(this.languageBox.getItemCount() - 1);
                    setLocaleCookie(localeName);
                }
            }
        }
        this.languageBox.addChangeHandler(event -> {
            String localeName = this.languageBox.getValue(this.languageBox.getSelectedIndex());
            setLocaleCookie(localeName);
    ...
    private void initAuthenticationHandlers(final GwtLoginInfo loginInfo) {
        ...
        authenticationMethod.addChangeHandler(e -> setAuthenticationMethod(authenticationMethod.getSelectedValue()));
    ...
    public void addAuthenticationHandler(final AuthenticationHandler authenticationHandler) {
        ...
        this.authenticationMethod.addItem(authenticationHandler.getLabel(), name);
